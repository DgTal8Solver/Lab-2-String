...
MyString::MyString(size_t count, char ch) : m_size(count), m_str(new char[m_size + 1]{'\0'}) {
    std::fill(m_str, m_str + m_size, ch);
}

MyString::MyString(const char* str) : m_size(strlen(str)), m_str(new char[m_size + 1]{'\0'}) {
